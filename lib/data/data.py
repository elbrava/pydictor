...
def init_pystrs():
    ...
    pystrs.sedb_dict = OrderedDict([
        ...
        (pystrs.sedb_range[8], []), (pystrs.sedb_range[9], []), (pystrs.sedb_range[10], []),
        (pystrs.sedb_range[11], []),
...
def init_pyoptions():
    ...
    pyoptions.count_switcher = 100000000000000000000000000
    ...
    for encode_file_name in os.listdir(paths.encode_function_path):
        ...
        if encode_name.endswith(pyoptions.encode_ending):
            pyoptions.operator[encode_name[:-len(pyoptions.encode_ending)].lower()] = getattr(
                __import__('lib.encode.' + encode_name, fromlist=True), encode_name)
    ...
    for tool_name in pyoptions.tool_range:
        for tool_enter in dir(
                __import__(str(tool_name), globals(), locals(), [str(tool_name) + pyoptions.tool_ending], )):
    ...
    for plug_name in pyoptions.plug_range:
        for plug_magic in dir(
                __import__(str(plug_name), globals(), locals(), [str(plug_name) + pyoptions.plug_ending], )):
